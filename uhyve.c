...
static void uhyve_exit(void *arg) {
	...
	if (vcpu_threads) {
		for (uint32_t i = 0; i < ncores; i++) {
			if (pthread_self() == vcpu_threads[i]) continue;

			pthread_kill(vcpu_threads[i], SIGTERM);
		}

		if (netfd > 0) pthread_kill(net_thread, SIGTERM);
	}
...
static int vcpu_loop(void) {
	...
	if (uhyve_gdb_enabled) {
		if (cpuid == 0) uhyve_gdb_init(vcpufd);
	...
	while (1) {
		...
		if (ret == -1) {
...
#ifdef __x86_64__
				err(1,
					"KVM: host/guest translation fault: rip=0x%llx",
					regs.rip);
#else
				err(1,
					"KVM: host/guest translation fault: elr_el1=0x%llx",
					regs.elr_el1);
#endif
			}
			...
			default:
				err(1,
					"KVM: ioctl KVM_RUN in vcpu_loop for cpuid %d failed",
					cpuid);
				break;
			}
		}
		...
		case KVM_EXIT_HLT:
			...
			if (uhyve_gdb_enabled) uhyve_gdb_handle_term();
			return 0;
		...
		case KVM_EXIT_IO:
			...
			case UHYVE_UART_PORT:
				if (verbose) putc((unsigned char)raddr, stderr);
				break;
			case UHYVE_PORT_WRITE: {
				uhyve_write_t *uhyve_write =
					(uhyve_write_t *)(guest_mem + raddr);
				...
				while (bytes_to_write > 0) {
					...
					virt_to_phys((size_t)uhyve_write->buf + bytes_written,
								 &physical_address,
								 &physical_address_end);

					size_t bytes_to_write_step =
						min(physical_address_end - physical_address,
							bytes_to_write);
					size_t bytes_written_step =
						write(uhyve_write->fd,
							  guest_mem + physical_address,
							  bytes_to_write_step);
					bytes_written += bytes_written_step;
					if (bytes_written_step < bytes_to_write_step) break;
			...
			case UHYVE_PORT_READ: {
				...
				while (bytes_to_read > 0) {
					size_t physical_address;
					size_t physical_address_end;
					virt_to_phys((size_t)uhyve_read->buf + bytes_read,
								 &physical_address,
								 &physical_address_end);

					size_t bytes_to_read_step = min(
						physical_address_end - physical_address, bytes_to_read);
					size_t bytes_read_step = read(uhyve_read->fd,
												  guest_mem + physical_address,
												  bytes_to_read_step);
					bytes_read += bytes_read_step;
					if (bytes_read_step < bytes_to_read_step) break;
			...
			case UHYVE_PORT_OPEN: {
				...
				if (realpath((const char *)guest_mem + (size_t)uhyve_open->name,
					...
					uhyve_open->ret =
						open((const char *)guest_mem + (size_t)uhyve_open->name,
							 uhyve_open->flags,
							 uhyve_open->mode);
				break;
			}
			...
			case UHYVE_PORT_CLOSE: {
				uhyve_close_t *uhyve_close =
					(uhyve_close_t *)(guest_mem + raddr);
			...
			case UHYVE_PORT_NETINFO: {
				uhyve_netinfo_t *uhyve_netinfo =
					(uhyve_netinfo_t *)(guest_mem + raddr);
			...
			case UHYVE_PORT_NETWRITE: {
				uhyve_netwrite_t *uhyve_netwrite =
					(uhyve_netwrite_t *)(guest_mem + raddr);
				size_t len = 0;
				while (len < uhyve_netwrite->len) {
					ret = write(netfd,
								guest_mem + (size_t)uhyve_netwrite->data + len,
			...
			case UHYVE_PORT_NETREAD: {
				uhyve_netread_t *uhyve_netread =
					(uhyve_netread_t *)(guest_mem + raddr);
				ret = read(netfd,
						   guest_mem + (size_t)uhyve_netread->data,
						   uhyve_netread->len);
				if (ret > 0) {
			...
			case UHYVE_PORT_NETSTAT: {
				uhyve_netstat_t *uhyve_netstat =
					(uhyve_netstat_t *)(guest_mem + raddr);
			...
			case UHYVE_PORT_LSEEK: {
				uhyve_lseek_t *uhyve_lseek =
					(uhyve_lseek_t *)(guest_mem + raddr);

				uhyve_lseek->offset = lseek(
					uhyve_lseek->fd, uhyve_lseek->offset, uhyve_lseek->whence);
				break;
			}
			...
			default:
				err(1,
					"KVM: unhandled KVM_EXIT_IO / KVM_EXIT_MMIO at port 0x%lx\n",
					port);
				break;
			}
			break;
		...
		case KVM_EXIT_FAIL_ENTRY:
			...
			err(1,
				"KVM: entry failure: hw_entry_failure_reason=0x%llx\n",
		...
		case KVM_EXIT_INTERNAL_ERROR:
			if (uhyve_gdb_enabled)
				uhyve_gdb_handle_exception(vcpufd, GDB_SIGNAL_SEGV);
			err(1,
				"KVM: internal error exit: suberror = 0x%x\n",
		...
		default:
			fprintf(stderr,
					"KVM: unhandled exit: exit_reason = 0x%x\n",
					run->exit_reason);
			exit(EXIT_FAILURE);
		}
	}
...
static int vcpu_init(void) {
	...
	if (run == MAP_FAILED) err(1, "KVM: VCPU mmap failed");
...
void determine_guest_allocations(void) {
	...
	if (write(mig_efd, &event_counter, sizeof(event_counter)) < 0) {
		fprintf(stderr,
				"[ERROR] Could not request the guest's free "
				"list - %d (%s) - Abort!\n",
				errno,
				strerror(errno));
		return;
...
void sigterm_handler(int signum) { pthread_exit(0); }
...
int uhyve_init(char *path) {
	...
	if (start_mig_server) {
		...
		if (hermit_memory) guest_size = memparse(hermit_memory);

		const char *hermit_cpus = getenv("HERMIT_CPUS");
		if (hermit_cpus) ncores = (uint32_t)atoi(hermit_cpus);

		const char *full_chk = getenv("HERMIT_FULLCHECKPOINT");
		if (full_chk && (strcmp(full_chk, "0") != 0)) full_checkpoint = true;
	}

	vcpu_threads = (pthread_t *)calloc(ncores, sizeof(pthread_t));
	if (!vcpu_threads) err(1, "Not enough memory");

	vcpu_fds = (int *)calloc(ncores, sizeof(int));
	if (!vcpu_fds) err(1, "Not enough memory");

	kvm = open("/dev/kvm", O_RDWR | O_CLOEXEC);
	if (kvm < 0) err(1, "Could not open: /dev/kvm");

	/* Make sure we have the stable version of the API */
	int kvm_api_version = kvm_ioctl(kvm, KVM_GET_API_VERSION, NULL);
	if (kvm_api_version != 12)
		err(1,
			"KVM: API version is %d, uhyve requires version 12",
			kvm_api_version);
	...
	init_kvm_arch();
	if (restart) {
		if (load_checkpoint(guest_mem, path) != 0) exit(EXIT_FAILURE);
	} else if (start_mig_server) {
		load_migration_data(guest_mem);
		close_migration_channel();
	} else {
		if (load_kernel(guest_mem, path) != 0) exit(EXIT_FAILURE);
	}
#endif
	...
	if (netif_str) {
		...
		if (netfd < 0) err(1, "unable to initialized network");
	}
...
int uhyve_loop(int argc, char **argv) {
	...
	uhyve_argv = &argv[1];
	uhyve_envp = environ;
	while (uhyve_envp[i] != NULL) i++;
	uhyve_envc = i;

	if (uhyve_argc > MAX_ARGC_ENVC) {
		fprintf(stderr,
				"uhyve downsiize envc from %d to %d\n",
				uhyve_argc,
				MAX_ARGC_ENVC);
	...
	if (uhyve_argc > MAX_ARGC_ENVC || uhyve_envc > MAX_ARGC_ENVC) {
		fprintf(
			stderr,
			"uhyve cannot forward more than %d command line "
	...
	if (hermit_check) ts = atoi(hermit_check);
	...
	if (hermit_mig_support) {
		...
		pthread_create(
			&sig_thr_id, NULL, migration_handler, (void *)&signal_mask);
		...
		fprintf(stderr,
				"[INFO] Creating eventfd for migration "
				"requests\n");
		if ((mig_efd = eventfd(0, 0)) < 0) {
			fprintf(stderr,
					"[WARNING] Could create the migration "
					"eventfd - %d (%s).\n",
					errno,
					strerror(errno));
		}
	...
	init_kvm_arch();
	if (restart) {
		if (load_checkpoint(guest_mem, guest_path) != 0) exit(EXIT_FAILURE);
	} else {
		if (load_kernel(guest_mem, guest_path) != 0) exit(EXIT_FAILURE);
	}
#endif
